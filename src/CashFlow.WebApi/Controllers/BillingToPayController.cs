...
public class BillingToPayController : ControllerBase
{
    ...
    public async Task<IActionResult> WriteOffAsync([FromBody]BillToPayDto bill)
    {
        ...
        _logger.LogInformation("The user ${User} has been wrote off the bill to pay title ${@Bill}", HttpContext?.User?.Identity?.Name, bill);
        return NoContent();
    }
    ...
    public async Task<IActionResult> DeleteAsync(int id)
    {
        ...
        _logger.LogInformation("The user ${User} has been deleted the bill to pay title ${Id}", HttpContext?.User?.Identity?.Name, id);
        return NoContent();
    }
    ...
    public async Task<IActionResult> AddNewAsync([FromBody]BillToPayInsertDto bill)
    {
        ...
        _logger.LogInformation("The user ${User} has been added the bill to pay title ${@Bill}", HttpContext?.User?.Identity?.Name, bill);
        return NoContent();
    }
}